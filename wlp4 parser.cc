...
void func(Node* p, vector<string>& v, VariableTable& vt){
    if (p->r.RHS.size() == 1){
...
void func2(Node* p, VariableTable& vt){

    if (p->r.RHS.size() == 0){
    }
    else{
        func2(p->children[0], vt);
    ...
    Procedure(Node* n){
        if (n->r.LHS == "procedure"){
            ...
            if (n->children[3]->r.RHS.size() != 0){
                func(n->children[3]->children[0], signature, vt);
            }
            func2(n->children[6], vt);
        }
        else{
...
void annotateTypes(Node* n, VariableTable& vt, ProcedureTable& pt){
    //cout << vt.table.size() << endl;
    for (int i = 0; i < n->children.size(); i++){
        annotateTypes(n->children[i], vt, pt);
    }
    if (n->t.kind == "NUM"){
        n->type = "int";
    }
...
void collectProcedures(Node* p, ProcedureTable& pt){
    if (p->r.RHS.size() == 1){
        ...
        annotateTypes(p->children[0], pr.vt, pt);
        ...
        collectProcedures(p->children[1], pt);
    }
}
...
int main(){
    ...
    ProcedureTable pt;
    collectProcedures(treeStack[0]->children[1], pt);
    for (int i =0; i < treeStack.size(); i++){
            delete treeStack[i];
    }
    }
